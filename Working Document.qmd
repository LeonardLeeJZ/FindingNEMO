...
multi_entity <- links %>%
  group_by(target) %>%
  filter(all(c("Beneficial Owner", "Company Contacts") %in% type)) %>%
  ungroup() %>%
  distinct(target)

anom_nodes <- anom_nodes %>%
  mutate(Ult_BO = ifelse(id %in% grouped_edges$target, "yes", NA)) %>%
  mutate(Multi_ent = ifelse(id %in% multi_entity$target, "yes", NA))
```
